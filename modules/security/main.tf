resource "aws_security_group" "alb" {
  ...
  description = "Internet traffic to the public load balancer"
  vpc_id      = var.vpc_id
...
resource "aws_vpc_security_group_egress_rule" "app_outbound" {
  security_group_id = aws_security_group.app.id
  description       = "Outbound package, AWS API, and database traffic"
  cidr_ipv4         = "0.0.0.0/0"
  ip_protocol       = "-1"
}
...
resource "aws_vpc_security_group_egress_rule" "alb_to_app" {
  security_group_id            = aws_security_group.alb.id
  description                  = "Forward requests to application instances"
...
resource "aws_security_group" "database" {
  ...
  description = "Database accepts traffic only from application instances"
  vpc_id      = var.vpc_id
...
resource "aws_kms_key" "this" {
  description             = "Encryption key for ${var.name} application and database data"
...
data "aws_iam_policy_document" "kms" {
  ...
  statement {
    sid       = "AllowAutoScalingPersistentResourceGrants"
  ...
  statement {
    ...
    condition {
      test     = "ArnLike"
      variable = "kms:EncryptionContext:${data.aws_partition.current.partition}:logs:arn"
